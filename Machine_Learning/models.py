...
class PerceptronModel(Module):
    ...
    def get_prediction(self, x):
        ...
        "*** YOUR CODE HERE ***"
        val = score.item() if hasattr(score, "item") else float(score)
...
class DigitClassificationModel(Module):
    ...
    def __init__(self):
        ...
        self.fc3 = Linear(hidden_size_2, output_size)
...
class LanguageIDModel(Module):
    ...
    def forward(self, xs):
        ...
        for i in range(1, len(xs)):
            # Standard RNN update: h_t = relu(W_x * x_t + W_h * h_{t-1})
            h = relu(self.char_layer(xs[i]) + self.hidden_layer(h))
...
def Convolve(input: tensor, weight: tensor):
    ...
    for r in range(output_height):
        for c in range(output_width):
            input_slice = input[r:r + weight_height, c:c + weight_width]
            convolution_sum = (input_slice * weight).sum()
            Output_Tensor[r, c] = convolution_sum
    "*** End Code ***"
...
class DigitConvolutionalModel(Module):
    ...
    def __init__(self):
        ...
        input_size_mlp = 676 
        hidden_size_1 = 128
        self.fc1 = Linear(input_size_mlp, hidden_size_1)
        self.fc2 = Linear(hidden_size_1, output_size)
