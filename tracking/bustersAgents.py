...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for dist in livingGhostPositionDistributions:
            ...
            ml = dist.argMax()
            if ml is not None:
                most_likely_positions.append(ml)

        if not most_likely_positions:
            return Directions.STOP

        target = min(most_likely_positions, key=lambda p: self.distancer.getDistance(pacmanPosition, p))

        best_action = Directions.STOP
        best_dist = float('inf')
        for action in legal:
            succ = Actions.getSuccessor(pacmanPosition, action)
            d = self.distancer.getDistance(succ, target)
            if d < best_dist:
                best_dist = d
                best_action = action
